      ...
      if (!controller.isHandled()) {                                                                          // 102
        ...
        if (state && state.initial === true) {                                                                // 110
          ...
          this.layout(this.lookupOption('layoutTemplate'), {data: {url: this.url}});                          // 116
                                                                                                              // 117
          var errorTemplate;                                                                                  // 118
                                                                                                              // 119
          if (self.routes.length === 0) {                                                                     // 120
            errorTemplate = this.lookupOption('noRoutesTemplate') || NO_ROUTES_TEMPLATE;                      // 121
          } else {                                                                                            // 122
            errorTemplate = this.lookupOption('notFoundTemplate') || DEFAULT_NOT_FOUND_TEMPLATE;              // 123
          }                                                                                                   // 124
                                                                                                              // 125
          this.render(errorTemplate, {data: {url: this.url}});                                                // 126
          this.renderRegions();                                                                               // 127
                                                                                                              // 128
          // kind of janky but will work for now. this makes sure                                             // 129
          // that any downstream functions see that this route has been                                       // 130
          // handled so we don't get into an infinite loop with the                                           // 131
          // server.                                                                                          // 132
          controller.isHandled = function () { return true; };                                                // 133
        }                                                                                                     // 134
                                                                                                              // 135
        return done && done.call(controller);                                                                 // 136
      }                                                                                                       // 137
    }                                                                                                         // 138
  });                                                                                                         // 139
                                                                                                              // 140
  // Note: even if the controller didn't actually change I change the                                         // 141
  // currentDep since if we did a dispatch, the url changed and that                                          // 142
  // means either we have a new controller OR the parameters for an                                           // 143
  // existing controller have changed.                                                                        // 144
  if (this._currentController == controller)                                                                  // 145
    this._currentDep.changed();                                                                               // 146
                                                                                                              // 147
  return controller;                                                                                          // 148
};                                                                                                            // 149
                                                                                                              // 150
/**                                                                                                           // 151
 * The current controller object.                                                                             // 152
 */                                                                                                           // 153
Router.prototype.current = function () {                                                                      // 154
  this._currentDep.depend();                                                                                  // 155
  return this._currentController;                                                                             // 156
};                                                                                                            // 157
                                                                                                              // 158
/*                                                                                                            // 159
 * Scroll to a specific location on the page.                                                                 // 160
 * Overridable by applications that want to customize this behavior.                                          // 161
 */                                                                                                           // 162
Router.prototype._scrollToHash = function (hashValue) {                                                       // 163
  try {                                                                                                       // 164
    var $target = $(hashValue);                                                                               // 165
    $('html, body').scrollTop($target.offset().top);                                                          // 166
  } catch (e) {                                                                                               // 167
    // in case the hashValue is bogus just bail out                                                           // 168
  }                                                                                                           // 169
};                                                                                                            // 170
                                                                                                              // 171
/**                                                                                                           // 172
 * Start reacting to location changes.                                                                        // 173
 */                                                                                                           // 174
Router.prototype.start = function () {                                                                        // 175
  var self = this;                                                                                            // 176
  var prevLocation;                                                                                           // 177
                                                                                                              // 178
  self._locationComputation = Deps.autorun(function onLocationChange (c) {                                    // 179
    var controller;                                                                                           // 180
    var loc = Iron.Location.get();                                                                            // 181
    var hash, pathname, search;                                                                               // 182
    var current = self._currentController;                                                                    // 183
                                                                                                              // 184
    if (!current || (prevLocation && prevLocation.path !== loc.path)) {                                       // 185
      controller = self.dispatch(loc.href, null, function onRouterStartDispatchCompleted (error) {            // 186
        // if we're going to the server cancel the url change                                                 // 187
        if (!this.isHandled()) {                                                                              // 188
          loc.cancelUrlChange();                                                                              // 189
          window.location = loc.path;                                                                         // 190
        }                                                                                                     // 191
      });                                                                                                     // 192
    } else {                                                                                                  // 193
      self._scrollToHash(loc.hash);                                                                           // 194
      // either the query or hash has changed so configure the current                                        // 195
      // controller again.                                                                                    // 196
      current.configureFromUrl(loc.href);                                                                     // 197
    }                                                                                                         // 198
                                                                                                              // 199
    prevLocation = loc;                                                                                       // 200
  });                                                                                                         // 201
};                                                                                                            // 202
                                                                                                              // 203
/**                                                                                                           // 204
 * Stop all computations and put us in a not started state.                                                   // 205
 */                                                                                                           // 206
Router.prototype.stop = function () {                                                                         // 207
  if (!this._isStarted)                                                                                       // 208
    return;                                                                                                   // 209
                                                                                                              // 210
  if (this._locationComputation)                                                                              // 211
    this._locationComputation.stop();                                                                         // 212
                                                                                                              // 213
  if (this._currentController)                                                                                // 214
    this._currentController.stop();                                                                           // 215
                                                                                                              // 216
  this._isStarted = false;                                                                                    // 217
};                                                                                                            // 218
                                                                                                              // 219
/**                                                                                                           // 220
 * Go to a given path or route name, optinally pass parameters and options.                                   // 221
 *                                                                                                            // 222
 * Example:                                                                                                   // 223
 * router.go('itemsShowRoute', {_id: 5}, {hash: 'frag', query: 'string});                                     // 224
 */                                                                                                           // 225
Router.prototype.go = function (routeNameOrPath, params, options) {                                           // 226
  var self = this;                                                                                            // 227
  var isPath = /^\/|http/;                                                                                    // 228
  var path;                                                                                                   // 229
                                                                                                              // 230
  options = options || {};                                                                                    // 231
                                                                                                              // 232
  if (isPath.test(routeNameOrPath)) {                                                                         // 233
    // it's a path!                                                                                           // 234
    path = routeNameOrPath;                                                                                   // 235
  } else {                                                                                                    // 236
    // it's a route name!                                                                                     // 237
    var route = self.routes[routeNameOrPath];                                                                 // 238
    assert(route, "No route found named " + JSON.stringify(routeNameOrPath));                                 // 239
    path = route.path(params, _.extend(options, {throwOnMissingParams: true}));                               // 240
  }                                                                                                           // 241
                                                                                                              // 242
  // let Iron Location handle it and we'll pick up the change in                                              // 243
  // Iron.Location.get() computation.                                                                         // 244
  Iron.Location.go(path, options);                                                                            // 245
};                                                                                                            // 246
                                                                                                              // 247
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
